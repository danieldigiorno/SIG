...
function actualizarPuntos(){
  ...
  while (j<puntos.length){
    ...
    if(j>0){
        $("<button class=" + "'btn btn-primary btn-link'" +" id=" + "up_btn_" + j + " onclick="+"subirPunto("+j+") style=\"float:right\" "+"><i class=\"glyphicon glyphicon-arrow-up\"></i></button>").appendTo("#elem_"+j);
    }
    if(j<puntos.length-1){
        $("<button class=" + "'btn btn-primary btn-link'" +" id=" + "down_btn_" + j + " onclick="+"bajarPunto("+j+") style=\"float:right\" "+"><i class=\"glyphicon glyphicon-arrow-down\"></i></button>").appendTo("#elem_"+j);
    }
    j++;
  }
}
